...
import { gitPrepare, gitPush } from "./tools/git-ops.js";
import { buildProject } from "./tools/build-project.js";
import { reportPlan, reportSummary } from "./tools/report.js";
import { fetchGithubIssue, createGithubIssue, createGithubPR, reviewGithubPR } from "./tools/github.js";
import { emitEvent } from "./tools/helpers.js";
...
2. mcp__dev-tools__report_plan으로 계획을 보고해라
3. mcp__dev-tools__git_prepare로 브랜치를 생성해라
4. 코드를 수정해라
5. mcp__dev-tools__build_project로 빌드를 검증해라
6. 빌드 실패 시 → 에러 분석 → 수정 → 다시 빌드 (최대 5회)
7. mcp__dev-tools__report_summary로 변경사항을 보고해라
8. mcp__dev-tools__git_push로 push해라
9. mcp__dev-tools__create_github_pr로 PR을 생성해라
...
3. mcp__dev-tools__review_github_pr로 리뷰를 남겨라
  - 문제 없으면: APPROVE
...
2. 개선할 점을 찾아라:
   ...
   - TODO/FIXME 주석
   - 보안 이슈, 하드코딩된 설정값
3. 발견한 항목마다 mcp__dev-tools__create_github_issue로 Issue를 생성해라
...
export async function runAgent(config: AgentConfig): Promise<void> {
  ...
  const dryRunBlockedTools = [
    "mcp__dev-tools__git_push",
    "mcp__dev-tools__git_prepare",
    "mcp__dev-tools__build_project",
    "mcp__dev-tools__create_github_pr",
  ];
    ...
    options: {
      ...
      hooks: {
        PreToolUse: [
          ...
          // push 전 빌드 검증
          {
            matcher: "mcp__dev-tools__git_push",
            hooks: [async () => {
              if (!buildVerified) {
                return {
                  hookSpecificOutput: {
